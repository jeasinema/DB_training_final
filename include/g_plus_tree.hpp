/*-----------------------------------------------------
 File Name : g_plus_tree.cpp
 Purpose :
 Creation Date : 19-06-2017
 Last Modified : Tue 20 Jun 2017 01:34:15 PM CST
 Created By : Jeasine Ma [jeasinema[at]gmail[dot]com]
-----------------------------------------------------*/
#ifndef G_PLUS_TREE_HPP
#define G_PLUS_TREE_HPP
...
const char Edge_File[]="./map/BJ.edge.p2p";//第一行两个整数n,m表示点数和边数，接下来m行每行三个整数U,V,C表示U->V有一条长度为C的边
const char Node_File[]="./map/BJ.node";//共N行每行一个整数两个实数id,x,y表示id结点的经纬度(但输入不考虑id，只顺序从0读到n-1，整数N在Edge文件里)
const char Map_File[]="./map/GP_Tree.data";//共N行每行一个整数两个实数id,x,y表示id结点的经纬度(但输入不考虑id，只顺序从0读到n-1，整数N在Edge文件里)
...
long long Additional_Memory=1000000;//用于构建辅助矩阵的额外空间(int)
...
void read()
{
	...
	for(i=0;i<G.m;i++)//读取边
	{
		...
		G.add(j,k,l);//双向边  // FIX: 1. double dirs 2. should not -1
	}
	fclose(in);
