/*-----------------------------------------------------
 ...
 Last Modified : Tue 20 Jun 2017 01:23:13 PM CST
...
#define FILE_NODE "./map/BJ.node"
#define FILE_EDGE "./map/BJ.edge"
...
#define FILE_NODES_GTREE_PATH "./map/cal.paths"
#define FILE_GTREE 			  "./map/cal.gtree"
#define FILE_ONTREE_MIND	  "./map/cal.minds"
// input (useless)
#define FILE_OBJECT "BJ.object"
...
// ----- for pre query init, OCCURENCE LIST in paper -----
	...
	vector<std::pair<int,int>> leafinvlist;
	vector<int> up_pos;
...
//  In fact, this should be done just for the query node when query, or just pre-done for all nodes
void pre_query(const vector<int>& select_points){
	// first clear all
	...
	for ( int i = 0; i < select_points.size(); i++ ){
		int current = Nodes[select_points[i]].gtreepath.back();
		// add leaf inv list
		int pos = lower_bound( GTree[current].leafnodes.begin(), GTree[current].leafnodes.end(), select_points[i] ) - GTree[current].leafnodes.begin();
		GTree[current].leafinvlist.push_back(std::make_pair(pos, i));		
		// recursive
		int child;
		while( current != -1 ){
			child = current;
			...
			if ( GTree[top.id].isleaf ){
				...
				if ( top.id == Nodes[locid].gtreepath[top.lca_pos] ){
					...
					for ( int i = 0; i < GTree[top.id].leafinvlist.size(); i++ ){
						cands.push_back( std::make_pair(GTree[top.id].leafnodes[GTree[top.id].leafinvlist[i].first], 
                                    GTree[top.id].leafinvlist[i].second) );
					}
                    vector<int> tmp;
                    tmp.clear();
                    for (auto& i : cands)
					...
					result = dijkstra_candidate( locid, tmp, Nodes );
					for ( int i = 0; i < cands.size(); i++ ){
						Status_query status = { cands[i].second, true, top.lca_pos, result[i] };
						pq.push_back(status);
				...
				// else do 
					...
					for ( int i = 0; i < GTree[top.id].leafinvlist.size(); i++ ){
						posa = GTree[top.id].leafinvlist[i].first;
						//vertex = GTree[top.id].leafnodes[posa];
                        vertex = GTree[top.id].leafinvlist[i].second;
						allmin = -1;
